 ...
 */ //src/main/java/supplychain/activiti/conf/DatabaseConfiguration.java
...
public class DatabaseConfiguration {
    ...
    public PlatformTransactionManager annotationDrivenTransactionManager() {
        ...
        public Liquibase liquibase() {
            log.info("Liquibase disabled — using existing schema (act6 on RDS).");

            try {
                // Simply return a dummy Liquibase instance that does nothing.
                // We must call a valid constructor, but skip update().
                DatabaseConnection connection = new JdbcConnection(dataSource().getConnection());
                Database db = DatabaseFactory.getInstance().findCorrectDatabaseImplementation(connection);

                // Create Liquibase instance (required by Spring) but skip update()
                Liquibase liquibase = new Liquibase(
                        "META-INF/liquibase/activiti-app-db-changelog.xml",
                        new ClassLoaderResourceAccessor(),
                        db
                );

                log.info("Liquibase bean registered — auto-update intentionally skipped.");
                return liquibase;

            } catch (Exception e) {
                log.warn("Liquibase skipped due to connection error: {}", e.getMessage());
                return null;
            }
        }
